...
void DrawSceneGL(void)													// 绘制场景
{
	...
	glTranslatef(0.0f, 0.0f, -7.0f);                  					// 向屏幕内移7.0f
	glColor3f(1.0f, 0.0f, 1.0f); 										// 设置当前颜色
	glRotatef(90.0f, 1.0f, 0.0f, 1.0f);									// 绕XOZ平面二、四象限的角平分线旋转90度
	DrawCube(1.5f, 1.5f, 1.5f);											// 绘制一个正方体
...
void Update(void)														// 在此处更新对消息的动作
{
	...
	{
		pKeyL = TRUE;													// pKeyL设为TRUE
		lighting = !lighting;											// 切换光源的TURE/FALSE
	}
	if (OGL_keys->keyDown['L'] == FALSE)								// L键是否松开
	{
		pKeyL = FALSE;													// pKeyL设为FALSE
	}

	if (OGL_keys->keyDown['C'] == TRUE && !pKeyC)
	{
		pKeyC = TRUE;													// pKeyC设为TRUE
		light = !light;													// 切换两个光源
	}
	if (OGL_keys->keyDown['C'] == FALSE)								// C键是否松开
	{
		pKeyC = FALSE;													// pKeyC设为FALSE
	}
