...
BOOL LoadTextureGL(void)												// 加载图片并转换为纹理
{
	...
	// 加载图片并检查是否出错 ，如果图片不存在则返回	
	 if ((TextureImage[0]=LoadImage("Data/Soap.bmp"))
		&&(TextureImage[1]=LoadImage("Data/Top.bmp"))
		&&(TextureImage[2]=LoadImage("Data/Side.bmp")))
	{   
...
void DrawSceneGL(void)													// 绘制场景
{
	...
	// 在此处添加代码进行绘制:
	glTranslatef(0.0f, 0.0f,-8.0f);										// 移入屏幕8.0f
	glRotatef(angle_X, 1.0f, 0.0f, 0.0f);								// 使物体绕X轴旋转
	glRotatef(angle_Y, 0.0f, 1.0f, 0.0f);								// 使物体绕Y轴旋转

	// 绘制里面的肥皂
	// 绘制4个球
	glPushMatrix();														// 保存模型视图矩阵
		glTranslatef(-1.0f, 0.5f, 0.0f);								// 移动坐标系
		auxSolidSphere(0.5f);											// 绘制半径为0.5f的球
		glTranslatef(0.0f,-1.0f ,0.0f);									// 移动坐标系
		auxSolidSphere(0.5f);											// 绘制半径为0.5f的球
		glTranslatef(2.0f, 0.0f, 0.0f);									// 移动坐标系
		auxSolidSphere(0.5f);											// 绘制半径为0.5f的球
		glTranslatef(0.0f, 1.0f, 0.0f);									// 移动坐标系
		auxSolidSphere(0.5f);											// 绘制半径为0.5f的球
	...
	glPushMatrix();														// 保存模型视图矩阵
		glTranslatef(-1.0f,-0.5f, 0.0f);								// 移动坐标系
		auxSolidCylinder( 0.5f, 1.0);									// 绘制高为1.0f的圆锥
		glTranslatef( 2.0f, 0.0f, 0.0f);								// 移动坐标系
		auxSolidCylinder( 0.5f, 1.0f);									// 绘制高为1.0f的圆锥
	glPopMatrix();														// 恢复模型视图矩阵
	// 绘制横向2个圆柱
	glPushMatrix();														// 保存模型视图矩阵
		glRotatef( 90.0f, 0.0f, 0.0f, 1.0f);							// 旋转90度
		glTranslatef(-0.5f, 0.0f, 0.0f);								// 移动坐标系
		auxSolidCylinder( 0.5f, 2.0f);									// 绘制高为2.0f的圆锥
		glTranslatef( 1.0f, 0.0f, 0.0f);								// 移动坐标系
		auxSolidCylinder( 0.5f, 2.0f);									// 绘制高为2.0f的圆锥
	glPopMatrix();;														// 恢复模型视图矩阵
	...
	glBegin(GL_QUADS);
		// 肥皂的上表面
		glTexCoord2f(0.0f, 0.0f); glVertex3f(-1.0f, -0.5f,  0.5f);
		glTexCoord2f(1.0f, 0.0f); glVertex3f( 1.0f, -0.5f,  0.5f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f( 1.0f,  0.5f,  0.5f);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(-1.0f,  0.5f,  0.5f);
		// 肥皂的下表面
		glTexCoord2f(1.0f, 0.0f); glVertex3f(-1.0f, -0.5f, -0.5f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f(-1.0f,  0.5f, -0.5f);
		glTexCoord2f(0.0f, 1.0f); glVertex3f( 1.0f,  0.5f, -0.5f);
	...
	// 绘制肥皂的盒子
	glTranslatef(boxpos, 0.0f, 0.0f);									// 移走盒子

	glBindTexture(GL_TEXTURE_2D,texture[1]);							// 选择纹理
	glBegin(GL_QUADS);
		// 肥皂盒子的上表面
		glTexCoord2f(0.0f, 0.0f); glVertex3f(-1.6f, -1.1f,  0.6f);
		glTexCoord2f(1.0f, 0.0f); glVertex3f( 1.6f, -1.1f,  0.6f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f( 1.6f,  1.1f,  0.6f);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(-1.6f,  1.1f,  0.6f);
		// 肥皂盒的下表面
		glTexCoord2f(0.0f, 0.0f); glVertex3f(-1.6f,  1.1f, -0.6f);
		glTexCoord2f(1.0f, 0.0f); glVertex3f( 1.6f,  1.1f, -0.6f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f( 1.6f, -1.1f, -0.6f);
	...
	glPushMatrix();														// 保存模型视图矩阵
		glTranslatef( 1.6f, 0.0f, 0.6f);								// 移动坐标系
		glRotatef(rot, 0.0f, -1.0f, 0.0f);								// 绕y轴旋转
		// 肥皂盒可以打开的侧面
		glBegin(GL_QUADS);
			glTexCoord2f(0.0f, 0.0f); glVertex3f( 0.0f, -1.1f, -1.2f);
			glTexCoord2f(1.0f, 0.0f); glVertex3f( 0.0f,  1.1f, -1.2f);
			glTexCoord2f(1.0f, 1.0f); glVertex3f( 0.0f,  1.1f,  0.0f);
			glTexCoord2f(0.0f, 1.0f); glVertex3f( 0.0f, -1.1f,  0.0f);
		glEnd();
	glPopMatrix();														// 恢复模型视图矩阵
	...
	//打开肥皂盒
	if (openbox)														// 判断是否要打开盒子
	{
		if(boxpos > -4.0f)												// 盒子未移到打开后的最终位置？
		{
			if (rot <= 180.0f)											// 侧面没完全打开？
				rot += 5.0f;											// 打开侧面
			else
				boxpos -= 0.1f;											// 移开盒子
		}
		else
			boxpos = -4.0f;												// 将盒子固定在x=-4.0位置
	}
	//关闭肥皂盒
	else
	{
		if (rot > 0.0f)													// 侧面没完全关闭？
		{
			if (boxpos <= 0.0f)											// 盒子不在原点？
				boxpos += 0.1f;											// 移回盒子
			else
				rot -= 5.0f;											// 关闭侧面
		}
		else
			rot = 0.0f;													// 保证侧面处完全关闭状态
	}
...
void Update(void)														// 在此处更新对消息的动作
{
	...
	//打开肥皂盒
	if (OGL_keys->keyDown['O'] == TRUE)									// 判断O键是否按下
	{
		openbox = TRUE;
	}
	//关闭肥皂盒
	if (OGL_keys->keyDown['P'] == TRUE)									// 判断P键是否按下
	{
		openbox = FALSE;
	}
}
