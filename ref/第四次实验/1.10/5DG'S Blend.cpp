...
#define TITLE		"5DG'S Blend"										// 定义窗口标题
...
BOOL LoadTextureGL(void)												// 加载图片并转换为纹理
{
	...
	if ((TextureImage[0] = LoadImage("Data/Scene1.bmp"))
		&& (TextureImage[1] = LoadImage("Data/Scene2.bmp")))
	{   
...
// 定义绘制函数
void DrawPic(float pos_x, float pos_y, float pos_z, float offset_x, float offset_y)
{
	glPushMatrix();														// 当前模型矩阵进栈
	glTranslatef(pos_x, pos_y, pos_z);									// 设置四边形位置
	glBegin(GL_QUADS);													// 绘制四边形
		glNormal3f( 0.0f, 0.0f, 1.0f);									// 设置法向量
		glTexCoord2f(0.0f, 0.0f); glVertex2f(-offset_x, -offset_y);		// Offset_X和Offset_Y
		glTexCoord2f(1.0f, 0.0f); glVertex2f( offset_x, -offset_y);		// 控制平面大小
		glTexCoord2f(1.0f, 1.0f); glVertex2f( offset_x,  offset_y);
		glTexCoord2f(0.0f, 1.0f); glVertex2f(-offset_x,  offset_y);
	glEnd();															// 结束绘制
	glPopMatrix();														// 出栈恢复保存的模型矩阵
}
...
BOOL Initialize(GL_Window* window, Keys* keys)							// 用户绘制场景相关的函数在此处定义	
{
	...
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);				 	// 设置混合函数(new)
...
void DrawSceneGL(void)													// 绘制场景
{
	...
	//在此处添加代码进行绘制:
	glTranslatef(0.0f, 0.0f, -5.0f);									// 向屏幕内移动
	
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);									// 全亮度，不透明
	glBindTexture(GL_TEXTURE_2D, texture[0]);							// 选择纹理贴图0
	DrawPic(0.0f, 0.0f, -1.0f, 3.2f, 2.4f);								// 绘制背景贴图
	
	// 控制将要被透明的贴图在Z轴上的移动，比较效果的区别
	glColor4f(1.0f, 1.0f, 1.0f, blendAlpha);							// 全亮度，调整透明度
	glBindTexture(GL_TEXTURE_2D, texture[1]);							// 选择纹理贴图1
	DrawPic(pos_x, 0.0f, pos_z, 2.7f, 2.0f);							// 绘制将要被透明的贴图
...
void Update(void)														// 在此处更新对消息的动作
{
	...
	// 控制透明度
	if (OGL_keys->keyDown[VK_UP] == TRUE)								// 向上方向键按下？
	{
		blendAlpha += 0.01f;			 								// 增加透明度
	}
	if (OGL_keys->keyDown[VK_DOWN] == TRUE)								// 向下方向键按下？
	{
		blendAlpha -= 0.01f;			 								// 降低透明度
	}
	
	// 控制将要被透明的贴图移动
	if (OGL_keys->keyDown[VK_RIGHT] == TRUE)							// 向右移动
	{
		pos_x += 0.03f;
	}
	if (OGL_keys->keyDown[VK_LEFT] == TRUE)								// 向左移动
	{
		pos_x -= 0.03f;
	}
	if (OGL_keys->keyDown[VK_PRIOR] == TRUE)							// 向内移动
	{
		pos_z += 0.03f;
	}
	if (OGL_keys->keyDown[VK_NEXT] == TRUE)								// 向外移动
	{
		pos_z -= 0.03f;
	}
	
	// 控制贴图透明的开与关
	if (OGL_keys->keyDown['B'] == TRUE && !pKeyB)						// 判断B键是否被按下
	{
		pKeyB = TRUE;
		blend = !blend;
		if (blend)
